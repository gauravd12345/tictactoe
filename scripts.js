...
function placeMove(move){
    if(!gameOver){
        ...
        if(count <= 9){
            var result = AIMove();
            setPiece(board, "O", result[0], result[1]); 
            printBoard();
            displayStatus();
        }
...
function runGameStates(board, player, layer){
    ...
    for(var i = 0; i < availableMoves.length; i++){
        //console.log(board, player, availableMoves[i][0], availableMoves[i][1]);
        board = setPiece(board, player, availableMoves[i][0], availableMoves[i][1]);
        var score = 0;
        if(player == "X"){
            score = max(runGameStates(board, "O", layer - 1));

        }
        else{
            score = min(runGameStates(board, "X", layer - 1));

        }
        scoreList.push(score);
        //console.log(score, layer, scoreList);
        board = setPiece(board, " ", availableMoves[i][0], availableMoves[i][1]);
    }
...
function AIMove(){
    var result = runGameStates(board, "O", 8);
    var availableMoves = getAvailableMoves(board);
    var index = 0;
